...
const cats = ['All', 'Publication', 'Competition', 'Research', 'Sports', 'Award', 'MUN']
const catColor = (cat: string) => {
  const map: Record<string, string> = { All: 'bg-neutral-800 text-white', Publication: 'bg-blue-600 text-white', Competition: 'bg-amber-500 text-white', Research: 'bg-purple-600 text-white', Sports: 'bg-green-600 text-white', Award: 'bg-red-600 text-white', MUN: 'bg-teal-600 text-white' }
  return map[cat] ?? 'bg-neutral-100 text-neutral-500'
}

export default function LawStudentAchievements() {
  const router = useRouter()
  const school = schools.find(s => s.id === 'law')!
  const data = school.studentAchievements ?? []
    ...
    <div className="min-h-screen bg-neutral-100 pb-24">
      <div className="max-w-md mx-auto bg-white min-h-screen flex flex-col">
        ...
        <div className="flex-1 bg-neutral-50 overflow-y-auto px-4 py-4 space-y-3">
          {filtered.length === 0 && <div className="text-center py-16"><Trophy className="w-10 h-10 text-neutral-200 mx-auto mb-3" /><p className="text-sm text-neutral-400">No achievements in this category.</p></div>}
          {filtered.map((item, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}
              className="bg-white rounded-2xl border border-neutral-100 shadow-sm overflow-hidden flex">
              <div className="w-12 bg-secondary/10 flex items-center justify-center shrink-0"><span className="text-xl">{item.badge}</span></div>
