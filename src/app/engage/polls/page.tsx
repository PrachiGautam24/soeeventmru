...
type Option = { id: string; label: string; emoji: string | null; votes: number }
type Poll = {
  id: string
  question: string
...
export default function PollsPage() {
  ...
  async function vote(poll: Poll, optionId: string) {
      ...
      prev.map((p) =>
        p.id === poll.id
          ? {
              ...
              options: p.options.map((o) => ({ ...o, votes: d.results.find((r: { id: string; votes: number }) => r.id === o.id)?.votes ?? o.votes })),
            }
          : p
      )
    )
    if (d.reward) {
      ...
      showReward({ xp: d.reward.xpDelta, coins: d.reward.coinsDelta, leveledUp: d.reward.leveledUp, level: d.reward.level, reason: 'Voted in a poll' })
    }
  }
    ...
    <>
      <header className="px-4 pt-10 pb-3 flex items-center gap-3">
        ...
        <div className="flex-1">
          <h1 className="text-lg font-black text-neutral-900">Campus Polls</h1>
          <p className="text-[11px] text-neutral-500">Your voice, live results, instant XP.</p>
        </div>
      ...
      <section className="mx-4 mt-2 flex flex-col gap-3">
        {loading && Array.from({ length: 2 }).map((_, i) => (
          <div key={i} className="h-48 rounded-2xl bg-white border border-neutral-100 animate-pulse" />
        ))}
        ...
        {polls.map((p) => (
          <PollCard key={p.id} poll={p} onVote={(opt) => vote(p, opt)} />
        ))}
      </section>
    </>
  )
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <p className="font-bold text-sm text-neutral-900">{poll.question}</p>
          {poll.description && <p className="text-[11px] text-neutral-500 mt-0.5">{poll.description}</p>}
        </div>
        <span className="text-[10px] font-bold text-primary shrink-0">+{poll.xpReward} XP</span>
      </div>
          ...
          return (
            <button
              ...
              className={`relative w-full text-left rounded-xl border overflow-hidden active:scale-[0.99] transition ${
                selected ? 'border-sky-400 bg-sky-50' : 'border-neutral-200 bg-white'
              }`}
            >
              {voted && (
                <motion.div
                  ...
                  className={`absolute inset-y-0 left-0 ${selected ? 'bg-sky-200/60' : 'bg-neutral-100'}`}
                />
              )}
              <div className="relative flex items-center gap-2 px-3 py-2.5">
                {o.emoji && <span className="text-base">{o.emoji}</span>}
                <span className={`flex-1 text-sm font-semibold ${selected ? 'text-sky-700' : 'text-neutral-800'}`}>
                  {o.label}
                </span>
                {voted && <span className="text-[11px] font-black text-neutral-700">{pct}%</span>}
              </div>
            </button>
          )
        })}
      ...
      <div className="flex items-center justify-between mt-3 text-[10px] text-neutral-500">
        <span className="flex items-center gap-1">
          <Users className="w-3 h-3" /> {poll.totalVotes} votes
        </span>
        {closed && <span className="text-rose-500 font-bold">Closed</span>}
        {!closed && poll.myVote && <span className="text-emerald-600 font-bold">Vote recorded ✓</span>}
      </div>
    </motion.div>
  )
}
