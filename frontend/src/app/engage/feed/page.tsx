  ...
  async function createPost() {
    ...
    if (!res.ok) {
      push({
        ...
        detail: 'Your voice earns XP.',
      })
      return
    }

    const data: { post: Post; reward?: Reward } = await res.json()
      ...
      <header className="px-4 pt-10 pb-3 flex items-center gap-3 sticky top-0 bg-neutral-50/80 backdrop-blur z-10">
        ...
        <div className="flex-1">
          ...
          <p className="text-[11px] text-neutral-500">
            Share a moment — earn XP for every post & like.
          </p>
        </div>
      ...
      <section className="mx-4 rounded-2xl bg-white border border-neutral-100 shadow-sm p-3">
        <div className="flex items-start gap-3">
          <div className="w-9 h-9 rounded-full bg-neutral-100 overflow-hidden flex items-center justify-center">
            {me?.image ? (
              <Image src={me.image} alt="me" width={36} height={36} />
            ) : (
      ...
      <section className="mt-3 flex flex-col gap-3 pb-4">
        {loading &&
          Array.from({ length: 3 }).map((_, i) => (
            <div
              key={i}
              className="mx-4 h-32 rounded-2xl bg-white border border-neutral-100 animate-pulse"
            />
          ))}
        ...
        {posts.map((p) => (
          <PostCard
            ...
            onLike={() => toggleLike(p)}
            onToggleComments={() =>
              setCommentsFor(commentsFor === p.id ? null : p.id)
            }
...
function PostCard({
  ...
  return (
    ...
    >
      <header className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <p className="font-bold text-sm truncate text-neutral-900">
      ...
      <p className="text-sm text-neutral-800 mt-2 whitespace-pre-wrap leading-relaxed">
        {post.content}
      </p>
      ...
      <div className="mt-3 flex items-center gap-5 border-t border-neutral-100 pt-2">
        <button
          ...
          className={`flex items-center gap-1.5 text-xs font-bold transition ${
            post.likedByMe ? 'text-rose-500' : 'text-neutral-500'
          }`}
        >
          <motion.span
            animate={{
              scale: post.likedByMe ? [1, 1.3, 1] : 1,
            }}
          >
            <Heart
        ...
        {comments.map((c) => (
          <div key={c.id} className="flex items-start gap-2">
            <div className="w-6 h-6 rounded-full bg-neutral-100 overflow-hidden flex items-center justify-center shrink-0">
              {c.author.image ? (
                <Image src={c.author.image} alt="" width={24} height={24} />
              ) : (
