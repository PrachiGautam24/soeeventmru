      ...
      <div className="max-w-md mx-auto min-h-screen bg-white shadow-xl flex flex-col">
        ...
        <div className="relative bg-secondary overflow-hidden">
          ...
          <div className="px-6 pt-10 pb-8 text-center">
            <h1 className="font-bold text-white text-xl leading-tight">Event Updates</h1>
            <p className="text-white/70 text-xs mt-1">{department.name}</p>
          </div>
          <div className="h-8">
            <svg viewBox="0 0 390 32" fill="none" xmlns="http://www.w3.org/2000/svg"
          ...
          {(['upcoming', 'completed'] as const).map(t => (
            <button
              ...
              className={`flex-1 py-3 text-sm font-semibold transition-all duration-200 ${
                tab === t ? 'text-secondary border-b-2 border-secondary' : 'text-neutral-400'
              }`}
            >
        ...
        <div className="flex-1 overflow-y-auto bg-neutral-50 px-4 py-4">
          <AnimatePresence mode="wait">
            {tab === 'upcoming' ? (
              ...
              >
                {completed.length === 0 ? (
                  <p className="text-sm text-neutral-400 text-center py-12">No completed events.</p>
                ) : (
                  completed.map((ev, i) => (
                    <motion.div
                      key={i}
                    ...
                    >
                      ...
                      {ev.slug === 'icass-2026' && (
                        <div className="relative h-36 bg-neutral-50 flex items-center justify-center border-b border-neutral-100">
                          ...
                          />
                          <span className="absolute top-3 right-3 bg-neutral-700 text-white text-[10px] font-semibold px-2.5 py-1 rounded-full">
                            Completed
                          </span>
                        </div>
                      )}
                      <div className="p-4 flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-semibold text-gray-800">{ev.title}</p>
                          {ev.description && (
                            <p className="text-xs text-neutral-500 mt-0.5 leading-snug">{ev.description}</p>
                          )}
                          <p className="text-xs text-neutral-400 mt-1 flex items-center gap-1">
                            <Calendar className="w-3 h-3" /> {ev.date}
                          </p>
                          {ev.slug && (
                            <span className="inline-block mt-2 text-xs font-semibold text-secondary">
                              View Details →
                            </span>
                          )}
                        </div>
