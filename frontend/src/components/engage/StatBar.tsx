...
export default function StatBar() {
  ...
  const progressPercent = me?.progress?.percent ?? 0
  const progressInLevel = me?.progress?.inLevel ?? 0
  const progressForLevel = me?.progress?.forLevel ?? 100
    ...
    return (
      ...
      >
        ...
        <p className="text-[11px] opacity-80">
          Join challenges, climb the leaderboard.
        </p>
      </Link>
    )
  }
      ...
      <div className="flex items-center gap-3 relative">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[10px] opacity-80 mt-1">
            {progressInLevel} / {progressForLevel} XP to Level{' '}
            {(me.level ?? 1) + 1}
          </p>
        </div>
      </div>
